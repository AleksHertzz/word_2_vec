...
for i in range(2571):
    try:
        a += 1
        path_to_file = tf.keras.utils.get_file('pass' + str(a) + '.txt',
                                               'https://tools.ietf.org/rfc/rfc' + str(a) + '.txt')
    except Exception:
        continue

first = True
with open("en.txt", "wt", encoding="cp1251") as out, fileinput.input(
        files=["pass{}.txt".format(i) for i in range(1, 2571)]) as f:
    for line in f:
        if "next" in line:
            if not first:
...
text_ds = tf.data.TextLineDataset('en.txt').filter(lambda x: tf.cast(tf.strings.length(x), bool))
